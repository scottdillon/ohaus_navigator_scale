...
' Public Declare PtrSafe Sub sleep Lib "kernel64" Alias "Sleep" (ByVal dwMilliseconds As Long)
'-------------------------------------------------------------------------------
...
'Declare PtrSafe Function BuildCommDCB Lib "kernel64" Alias "BuildCommDCBA" _
'    (ByVal lpDef As String, lpDCB As DCB) As Long
...
'Declare PtrSafe Function ClearCommError Lib "kernel64" _
'    (ByVal hFile As Long, lpErrors As Long, lpStat As COMSTAT) As Long
'
...
'Declare PtrSafe Function CloseHandle Lib "kernel64" (ByVal hObject As Long) As Long
...
'Declare PtrSafe Function CreateFile Lib "kernel64" Alias "CreateFileA" _
'    (ByVal lpFileName As String, ByVal dwDesiredAccess As Long, _
'    ByVal dwShareMode As Long, lpSecurityAttributes As Any, _
'    ByVal dwCreationDisposition As Long, ByVal dwFlagsAndAttributes As Long, _
'    ByVal hTemplateFile As Long) As Long
...
'Declare PtrSafe Function EscapeCommFunction Lib "kernel64" _
'    (ByVal nCid As Long, ByVal nFunc As Long) As Long
'
...
'Declare PtrSafe Function FormatMessage Lib "kernel64" Alias "FormatMessageA" _
'    (ByVal dwFlags As Long, lpSource As Any, ByVal dwMessageId As Long, _
'    ByVal dwLanguageId As Long, ByVal lpBuffer As String, ByVal nSize As Long, _
'    Arguments As Long) As Long
'
...
'Declare PtrSafe Function GetCommModemStatus Lib "kernel64" _
'    (ByVal hFile As Long, lpModemStat As Long) As Long
'
' Retrieves the current control settings for a specified
' communications device.
'
'Declare PtrSafe Function GetCommState Lib "kernel64" _
...
'Declare PtrSafe Function GetLastError Lib "kernel64" () As Long
'
...
'Declare PtrSafe Function GetOverlappedResult Lib "kernel64" _
'    (ByVal hFile As Long, lpOverlapped As OVERLAPPED, _
'    lpNumberOfBytesTransferred As Long, ByVal bWait As Long) As Long
'
...
'Declare PtrSafe Function PurgeComm Lib "kernel64" _
'    (ByVal hFile As Long, ByVal dwFlags As Long) As Long
'
...
'Declare PtrSafe Function ReadFile Lib "kernel64" _
'    (ByVal hFile As Long, ByVal lpBuffer As String, _
'    ByVal nNumberOfBytesToRead As Long, ByRef lpNumberOfBytesRead As Long, _
'    lpOverlapped As OVERLAPPED) As Long
'
...
'Declare PtrSafe Function SetCommState Lib "kernel64" _
'    (ByVal hCommDev As Long, lpDCB As DCB) As Long
'
' Sets the time-out parameters for all read and write operations on a
' specified communications device.
'
'Declare PtrSafe Function SetCommTimeouts Lib "kernel64" _
...
'Declare PtrSafe Function SetupComm Lib "kernel64" _
'    (ByVal hFile As Long, ByVal dwInQueue As Long, ByVal dwOutQueue As Long) As Long
'
...
Declare PtrSafe Function WriteFile Lib "kernel32" _
    ...
    lpOverlapped As OVERLAPPED) As Long
'Declare PtrSafe Function WriteFile Lib "kernel64" _
'    (ByVal hFile As Long, ByVal lpBuffer As String, _
'    ByVal nNumberOfBytesToWrite As Long, lpNumberOfBytesWritten As Long, _
'    lpOverlapped As OVERLAPPED) As Long
...
'   Error Code  - 0 = No Error.
'-------------------------------------------------------------------------------
Public Function CommRead(intPortID As Integer, ByRef strData As String, _
...
Public Sub read_navigator_scale()
    Dim intPortID As Integer ' Ex. 1, 2, 3, 4 for COM1 - COM4
    Dim lngStatus As Long
    Dim strData As String
    Dim strReadData As String
    Dim strReadDataTemp As String
    Dim i As Long
   ...
   'Writa data
    ...
    Do While (Len(strReadData) < 10 And i < 20)
        lngStatus = CommRead(intPortID, strReadDataTemp, 10)
        strReadData = strReadData & strReadDataTemp
        i = i + 1
        If i = 20 Then
            MsgBox "Cannot communicate with scale."
        End If
    Loop
    
    If lngStatus > 0 Then
        strReadData = Replace(strReadData, vbCrLf, "")
        strReadData = Replace(strReadData, "g", "")
        strReadData = Replace(strReadData, " ", "")
        strReadData = Replace(strReadData, "?", "")
        ActiveCell.Value = strReadData 'CDec(strReadData)
